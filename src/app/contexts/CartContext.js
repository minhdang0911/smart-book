...
export const CartProvider = ({ children }) => {
    ...
    const fetchCartData = useCallback(async (showLoading = true) => {
        ...
        try {
            ...
            const response = await fetch('https://smartbook-backend.tranminhdang.cloud/api/cart', {
                headers: {
    ...
    const updateItemQuantity = useCallback(
        async (itemId, newQuantity) => {
            ...
            try {
                const response = await fetch(`https://smartbook-backend.tranminhdang.cloud/api/cart/item/${itemId}`, {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    const removeItems = useCallback(
        async (itemIds) => {
            const token = localStorage.getItem('token');
            try {
                const response = await fetch('https://smartbook-backend.tranminhdang.cloud/api/cart/remove', {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    const clearCart = useCallback(async () => {
        ...
        try {
            ...
            const response = await fetch('https://smartbook-backend.tranminhdang.cloud/api/cart/clear', {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
                    'Content-Type': 'application/json',
