...
export const useReviewActions = () => {
    ...
    const checkCanReview = async (bookId, customMessage) => {
        ...
        try {
            ...
            do {
                const res = await fetch(`https://smartbook-backend.tranminhdang.cloud/api/orders?page=${page}`, {
                    method: 'GET',
                    headers: {
    ...
    const submitReview = async (bookId, rating, comment) => {
        ...
        const res = await fetch('https://smartbook-backend.tranminhdang.cloud/api/ratings', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${getToken()}`,
